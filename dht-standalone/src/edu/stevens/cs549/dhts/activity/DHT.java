...
public class DHT extends DHTBase implements IDHTResource, IDHTNode, IDHTBackground {
	...
	private static final Document doc = null;
	...
	private NodeInfo getSucc(NodeInfo info) throws Failed {
		...
		if (localInfo.addr.equals(info.addr)) {
			...
			try{
				//provides primary access to data of document.
				Document doc = getRequest(info.addr+UriApi.SUCC);
				//return all elements in the document with given a tag name
				NodeInfo succ= new NodeInfo(Integer.parseInt(doc.getElementsByTagName("id").item(0).getTextContent()), new URI(doc.getElementsByTagName("addr").item(0).getTextContent()));
				return succ;
			}
			catch(Exception e){
				throw new Failed("get successor node failes");
	...
	protected NodeInfo getPred(NodeInfo info) throws Failed {
		...
		if (localInfo.addr.equals(info.addr)) {
			 ...
			 */
			try{
				Document doc=getRequest(info.addr+UriApi.PRED);
				if(doc.getElementsByTagName("id").getLength()>=1){ 
				return new NodeInfo(Integer.parseInt(doc.getElementsByTagName("id").item(0).getTextContent()),new URI(doc.getElementsByTagName("addr").item(0).getTextContent()));
				}
			}
			catch (Exception e) {
				throw new Failed(String.format("get Predecessor node failed!", info.id));
			}
		}
		return null;
}
...
//		routing.joinCheck();
		...
		if (localInfo.equals(info)) {
			...
			if (IRouting.USE_FINGER_TABLE) {
				...
				for(int i= IRouting.NFINGERS-1;i>=1;i--){
					Document doc = getRequest(String.format(info.addr+UriApi.GET_NODE_BY_FINGER,i));
					int finger_id = Integer.parseInt(doc.getElementsByTagName("id").item(0).getTextContent());
					try{
						return new NodeInfo(finger_id,new URI(doc.getElementsByTagName("addr").item(0).getTextContent()));
					}
					catch (Exception e){
						System.err.println("Initialize NodeInfo error, the uri is wrong: "+e);
						}
					}
				return info;
		}
	...
	protected String[] get(NodeInfo n, String k) throws Failed {
		if (n.addr.equals(info.addr)) {
			 ...
			 */
			try{
			Document doc = getRequest(String.format(n.addr + UriApi.GET_KEY_VALUE, k));
			...
			for (int temp=0;temp < t;temp++) {
				...
				str[temp] = nlist.item(temp).getTextContent();
			}
			return str;
			}
			catch (Exception e) {
	...
	public void add(NodeInfo n, String k, String v) throws Failed {
		if (n.addr.equals(info.addr)) {
			 ...
			 * TODO: Do the Web service call.
			 */
			putRequest(String.format(n.addr + UriApi.SET_KEY_VALUE,k,v));
		}
	}
	...
	public void delete(NodeInfo n, String k, String v) throws Failed {
		if (n.addr.equals(info.addr)) {
			 ...
			 * TODO: Do the Web service call.
			 */
			deleteRequest(String.format(n.addr + UriApi.DELETE_KEY_VALUE, k, v));	
		}
	}
	 ...
	 */
		...
		try {
			Document doc = getRequest(String.format(uri + UriApi.FIND_SUCCESSOR, info.id));
			...
			succ = new NodeInfo(id, new URI(addr));
			setSucc(succ);
			stabilize();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public Document getRequest(String uri) {
		Document document = null;
		try {
			URI uris = new URI(uri);
			DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
			DocumentBuilder builder = factory.newDocumentBuilder();
			Response res = client.getRequest(uris);
			String str = res.readEntity(String.class);
			InputSource is = new InputSource(new StringReader(str));
			document = builder.parse(is);
			//doc.getDocumentElement().normalize();
		} catch (Exception e) {
			warning("Get request to " + uri + " failed. Error Message: " + e);
		}
		return document;
	}
	public void putRequest(String uri) {
		try {
		URI uris = new URI(uri);
		client.putRequest(uris, Entity.text(""));
		} catch (Exception ex) {
			 System.err.println("Put request  "+uri+ "Failed " + ex);
			ex.printStackTrace();
		}
		}
	public void deleteRequest(String uri) {
		try {
		URI uris = new URI(uri);
		client.deleteRequest(uris);
		} catch (Exception ex) {
			System.err.println("delete request  "+uri+ "Failed " + ex);
		ex.printStackTrace();
		}
		}
